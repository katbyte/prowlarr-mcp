...
func newGen(svc *definitions.Service, opts Options) (*gen, error) {
	if svc.Auth != "Prowlarr" {
		return nil, fmt.Errorf("%s: unknown Auth %q (want Prowlarr)", svc.Name, svc.Auth)
	}
...
//
// Dates are strings: Prowlarr sends RFC 3339 in UTC ("2026-09-19T13:00:00Z").
`)
