...
// Prowlarr's paths all start /api/v1 and run their words together; the
// prefix goes, the words are spelled, and a path outside the prefix keeps
// every segment.
func TestPathMethodNamePrefixAndWords(t *testing.T) {
	t.Parallel()

	words := map[string]string{"indexerproxy": "IndexerProxy", "testall": "TestAll", "downloadclient": "DownloadClient"}
	tests := []struct{ method, path, want string }{
		{"GET", "/api/v1/indexer/{id}", "GetIndexerById"},
		{"GET", "/api/v1/indexer", "GetIndexer"},
		{"DELETE", "/api/v1/indexerproxy/{id}", "DeleteIndexerProxyById"},
		{"POST", "/api/v1/downloadclient/testall", "PostDownloadClientTestAll"},
		{"PUT", "/api/v1/config/downloadclient/{id}", "PutConfigDownloadClientById"},
		// a parameter named like a word is still a parameter
		{"GET", "/api/v1/x/{indexerproxy}", "GetXByIndexerproxy"},
		// outside the prefix, and a prefix that is only the start of a segment
		{"GET", "/api", "GetApi"},
		{"GET", "/ping", "GetPing"},
		{"GET", "/api/v1x/thing", "GetApiV1xThing"},
		{"GET", "/{id}/api", "GetByIdApi"},
	}
	for _, tt := range tests {
		if got := pathMethodName(tt.method, tt.path, "/api/v1", words); got != tt.want {
			t.Errorf("pathMethodName(%s, %s) = %q, want %q", tt.method, tt.path, got, tt.want)
		}
	}
}
