...
pandorest generates the Prowlarr SDK, `lib/prowlarr`, from the OpenAPI document
in Prowlarr's own source tree.
...
taught Prowlarr's paging and authentication.

```
docs/prowlarr-openapi.json ── import ── api-definitions/prowlarr/*.json ── generate ── lib/prowlarr
                           (workarounds)          │
...
make pandorest-diff    # what docs/prowlarr-openapi.json changes against api-definitions/, breaking changes marked
...
- **Names.** Prowlarr's document has no operationIds, so methods are named
  after the method and path, with the `/api/v1` prefix left off and the
  run-together words of the paths spelled out (`config.Service.Words`):
  `GET /api/v1/indexer/{id}` is `GetIndexerById`, `PUT
  /api/v1/indexerproxy/{id}` is `PutIndexerProxyById`, and `GET /ping`,
  outside the prefix, is `GetPing`. Schema names lose their dots and
  underscores; fields keep the API's spelling (`AppProfileId`, `IndexerId`).
...
- **Operations.** Path parameters in template order; query and header
  ...
  the document says (Prowlarr's are all one key per value; the few it takes
  comma-separated, like the statistics filters, it declares as strings); the
  JSON request body or raw bytes; the success response (JSON, a file, or
  nothing); `ExpectedStatusCodes` from the 2xx responses; and `Pageable` for a
  GET with `page` and `pageSize` that answers `records` and `totalRecords`
...
- **Grouping.** One definitions file per spec tag, holding the tag's
  ...
  than one tag uses, `ProviderTestAllResult` and `Field` above all, is in
  `Common.json`. There is
...
with a `Name` (`prowlarr-created-accepted`), the `Service` it is for, the `Bug`
...
Prowlarr's document is written by Swashbuckle from the controllers, which
...
answer 201 and the updates 202, an action returning `object` or
`IActionResult` is documented as answering nothing, and a bulk write is
documented as answering one resource where it answers the list it changed.
Each table in `prowlarr.go` was read off the controllers in the release's
source and is proved by the integration suite against the same release. They
are listed, with what each one fixes, in [docs/README.md](../../docs/README.md).
...
`api-definitions/prowlarr/Service.json` names the package, the document's title
...
```
prowlarr: 1 added, 0 removed, 2 changed (1 breaking)
  document: Prowlarr 1.0.0 -> Prowlarr 1.0.0
+ operation GetIndexerByIdStatistics (GET /api/v1/indexer/{id}/statistics)
~ operation GetHistory (GET /api/v1/history)
    + option query indexerIds: List[Integer]
    ~ expected status codes [200] -> [200 204]
~ model IndexerResource
    ~ field Priority: Integer -> Integer64 [breaking]
```
...
```go
res, err := c.GetIndexerById(ctx, 7)
res.HttpResponse // set whenever the server answered, its body readable again
res.Model        // *prowlarr.IndexerResource

all, err := c.GetHistoryComplete(ctx, prowlarr.GetHistoryOperationOptions{EventType: []int{1}})
...
- A model the config names as written whole (`config.Service.WrittenWhole`:
  Prowlarr's four settings sections - host, UI, download client and
  development - each read with a GET and saved with a PUT of all of it)
  always sends its strings and numbers, empty or not. A field left out of
  such a PUT arrives as null and is saved as the old value rather than the
  `""` a caller cleared it to.
...
- A status outside `ExpectedStatusCodes` is a `*client.StatusError`, even a
  2xx, returned with the response; its message carries Prowlarr's own
...
Resource ID types and parsers (Prowlarr's ids are plain integers), per-model
...
(a Prowlarr command is polled by the tools, which know what they are waiting
