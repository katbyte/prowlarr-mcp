...
// around it: these start prowlarr-mcp as a client does, over stdio and over
...
// buildBinary builds prowlarr-mcp from this checkout. Under a coverage run it
...
func buildBinary(t *testing.T) (bin, coverDir string) {
	...
	bin = filepath.Join(t.TempDir(), "prowlarr-mcp")
	...
	if out, err := cmd.CombinedOutput(); err != nil {
		t.Fatalf("building prowlarr-mcp: %v\n%s", err, out)
	}
...
// binary is the built prowlarr-mcp and how to run it against the test server.
type binary struct {
	path, coverDir string
}

// command runs the binary in a directory and $HOME of its own, so a
// .prowlarr-mcp config on the machine running the tests is not read, with the
// connection from the suite's environment and no other PROWLARR_ setting
// unless given.
func (b binary) command(t *testing.T, env []string, args ...string) (*exec.Cmd, *lockedBuffer) {
	...
	for _, kv := range os.Environ() {
		if name, _, _ := strings.Cut(kv, "="); !strings.HasPrefix(name, "PROWLARR_") || slices.Contains([]string{"PROWLARR_SERVER", "PROWLARR_TOKEN"}, name) {
...
func connect(t *testing.T, transport mcp.Transport, stderr *lockedBuffer) *mcp.ClientSession {
	...
	if err != nil {
		t.Fatalf("connecting to prowlarr-mcp: %v\n%s", err, stderr)
	}
...
func TestTheBinary(t *testing.T) {
	...
	t.Run("version and info", func(t *testing.T) {
		...
		if out, err := cmd.Output(); err != nil || !strings.HasPrefix(string(out), "prowlarr-mcp ") {
			t.Errorf("version = %q, %v\n%s", out, err, stderr)
		}
		cmd, stderr = bin.command(t, nil, "info")
		if out, err := cmd.Output(); err != nil || !strings.Contains(string(out), `indexer "`+idxUsenet+`" (usenet`) || !strings.Contains(string(out), `application "`+appSonarr+`"`) {
			t.Errorf("info = %q, %v\n%s", out, err, stderr)
		}
	})
	...
	t.Run("stdio", func(t *testing.T) {
		for _, c := range []struct {
			...
			{"toolsets from the environment", []string{"PROWLARR_TOOLSETS=all", "PROWLARR_ENABLE_DELETE=true"}, nil, tools.Options{Toolsets: []string{"all"}, EnableDelete: true}},
			...
			t.Run(c.name, func(t *testing.T) {
				...
				if out := callOn(t, cs, "server_info", nil); !strings.HasPrefix(str(out["version"]), "2.") || num(t, out["indexers"], "indexers") == 0 {
		...
		t.Run("a write", func(t *testing.T) {
			cmd, stderr := bin.command(t, []string{"PROWLARR_TOOLSETS=all"}, "serve")
			cs := connect(t, &mcp.CommandTransport{Command: cmd}, stderr)
			t.Cleanup(func() { _ = cs.Close() })
			const label = "through-the-binary"
			t.Cleanup(func() { _, _ = invoke("tag_delete", map[string]any{"label": label}) })
			callOn(t, cs, "indexer_edit", map[string]any{"indexer": idxAnime, "add_tags": []string{label}})
			if !slices.Contains(strs(t, indexerRow(t, idxAnime)["tags"], "tags"), label) {
				t.Error("indexer_edit through the binary did not tag the indexer")
			}
			callOn(t, cs, "indexer_edit", map[string]any{"indexer": idxAnime, "remove_tags": []string{label}})
			if slices.Contains(strs(t, indexerRow(t, idxAnime)["tags"], "tags"), label) {
				t.Error("indexer_edit through the binary did not untag the indexer")
			}
		})
	})
	...
	t.Run("http", func(t *testing.T) {
		...
		cmd, stderr := bin.command(t, []string{"PROWLARR_TOOLSETS=curation", "PROWLARR_AUTH_TOKEN=" + token}, "serve", "--listen", addr)
		...
		if got, want := findings(t, callOn(t, cs, "audit_duplicates", nil)), []string{idxTorrent + "/duplicate"}; !slices.Equal(got, want) {
			t.Errorf("audit_duplicates over HTTP = %v, want %v", got, want)
		}
		_ = cs.Close()
	...
	t.Run("refuses to start", func(t *testing.T) {
		for _, c := range []struct {
			...
			{"without Prowlarr's API key", []string{"PROWLARR_TOKEN="}, []string{"serve"}, "token parameter can't be empty"},
