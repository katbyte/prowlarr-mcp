...
func run(t *testing.T, args ...string) (string, error) {
	...
	// a developer's own ~/.prowlarr-mcp must not leak into the run: with a
...
func TestMakeBuildsTheCommands(t *testing.T) {
	...
	out, err = run(t)
	if err != nil || !strings.Contains(out, "prowlarr-mcp help") {
...
func TestVersionCommand(t *testing.T) {
	...
	if err != nil || !strings.HasPrefix(out, "prowlarr-mcp ") {
...
func TestInfoNeedsConnectionParams(t *testing.T) {
	...
	_, err = run(t, "serve", "--server", "http://nas:9696")
	if err == nil || !strings.Contains(err.Error(), "token") {
		t.Errorf("serve with no token = %v, want a complaint about token", err)
	}
}

// `prowlarr-mcp tools` needs no server, so it has to work with nothing
...
func TestToolsCommand(t *testing.T) {
	...
	for _, want := range []string{"core", "server_info", "indexer_list", coreTool} {
		if !strings.Contains(out, want) {
...
func TestToolsCommandToolsets(t *testing.T) {
	...
	if !strings.Contains(out, "indexer_delete") || strings.Contains(out, "delete tools are hidden") {
