...
func TestNew(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ok", url: "http://nas:9696", auth: APIKey("t"), wantBase: "http://nas:9696"},
		{name: "trailing slash trimmed", url: "http://nas:9696/prowlarr/", auth: APIKey("t"), wantBase: "http://nas:9696/prowlarr"},
		{name: "missing url", url: "", auth: APIKey("t"), wantErr: "server URL is required"},
		{name: "no scheme", url: "nas:9696", auth: APIKey("t"), wantErr: "must include a scheme and host"},
		{name: "no host", url: "http://", auth: APIKey("t"), wantErr: "must include a scheme and host"},
		{name: "credentials in url", url: "http://kt:pw@nas:9696", auth: APIKey("t"), wantErr: "must not contain credentials"}, //nolint:gosec // the point of the case
		{name: "no authorizer", url: "http://nas:9696", wantErr: "authorizer is required"},
	}
...
func TestAPIKey(t *testing.T) {
	...
	if _, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v1/system/status", ExpectedStatusCodes: []int{http.StatusOK}}, nil); err != nil {
		t.Fatal(err)
	}
...
func TestOptionsObject(t *testing.T) {
	...
	if _, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v1/indexer", ExpectedStatusCodes: []int{http.StatusOK}, OptionsObject: opts}, nil); err != nil {
		t.Fatal(err)
	}

	if got.URL.Path != "/api/v1/indexer" || got.URL.Query().Get("Limit") != "5" {
...
func TestStatusError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "401 hint", status: http.StatusUnauthorized, body: "bad token", wantMsg: []string{"GET /api/v1/system/status: HTTP 401 (expected 200): bad token", "API key rejected"}},
		// Prowlarr answers a failed validation with the failures, one per field
		{
		...
		{name: "error model", status: http.StatusConflict, body: `{"message":"Indexer already exists","description":"stack"}`, wantMsg: []string{"HTTP 409 (expected 200): Indexer already exists"}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v1/system/status", ExpectedStatusCodes: []int{http.StatusOK}}, nil)

			se, ok := errors.AsType[*StatusError](err)
			if !ok {
				t.Fatalf("err = %v, want *StatusError", err)
			}
			if se.StatusCode != tt.status || StatusCode(err) != tt.status || se.Method != http.MethodGet || se.Path != "/api/v1/system/status" {
...
func TestBodies(t *testing.T) {
	...
	t.Run("json both ways, body readable again", func(t *testing.T) {
		...
		resp, err := execute(t, c, RequestOptions{ContentType: "application/json", HttpMethod: http.MethodPost, Path: "/api/v1/tag", ExpectedStatusCodes: []int{http.StatusOK}},
	...
	t.Run("bad json is an error", func(t *testing.T) {
		...
		var model struct{}
		if err := resp.Unmarshal(&model); err == nil || !strings.Contains(err.Error(), "GET /api/v1/system/status: decoding response") {
	...
	t.Run("raw body needs a concrete type for a range", func(t *testing.T) {
		...
		req, err := c.NewRequest(t.Context(), RequestOptions{ContentType: "image/*", HttpMethod: http.MethodPost, Path: "/api/v1/system/backup/restore/upload", ExpectedStatusCodes: []int{http.StatusNoContent}})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("stream left unread", func(t *testing.T) {
		...
		resp, err := execute(t, c, RequestOptions{HttpMethod: http.MethodGet, Path: "/api/v1/log/file/prowlarr.txt", ExpectedStatusCodes: []int{http.StatusOK}, StreamResponse: true}, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestListHelpers(t *testing.T) {
	...
	type kind string
	if got := CSV([]kind{"usenet", "torrent"}); got != "usenet,torrent" {
