// Package client is the hand-written base client the generated SDK, lib/prowlarr,
// sends every request through, after go-azure-sdk's sdk/client. Prowlarr takes
...
// APIKey authenticates to Prowlarr with the API key from Settings → General.
// Prowlarr also reads it from an apikey query parameter, which would put it in
...
// optionally the URL base Prowlarr is configured with, such as /prowlarr).
func New(baseURL string, authorizer Authorizer) (*Client, error) {
	...
	if err != nil || u.Scheme == "" || u.Host == "" {
		return nil, fmt.Errorf("server URL %q must include a scheme and host, e.g. http://nas:9696", baseURL)
	}
	if u.User != nil {
...
type StatusError struct {
	...
	Body string
	// Messages is what Prowlarr said went wrong, when the body is one of its
...
// validationFailure is one entry of the array Prowlarr answers a failed
...
// errorMessages reads what Prowlarr said went wrong out of an error body, or
