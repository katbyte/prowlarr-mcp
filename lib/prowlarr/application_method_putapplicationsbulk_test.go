...
func TestOperationPutApplicationsBulk(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "[{}]")
	result, err := c.PutApplicationsBulk(t.Context(), ApplicationBulkResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v1/applications/bulk")
