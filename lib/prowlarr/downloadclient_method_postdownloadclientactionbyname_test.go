...
func TestOperationPostDownloadClientActionByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostDownloadClientActionByName(t.Context(), "p/name", DownloadClientResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/downloadclient/action/p%2Fname")
