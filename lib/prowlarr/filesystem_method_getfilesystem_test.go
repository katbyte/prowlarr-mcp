...
func TestOperationGetFileSystem(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetFileSystem(t.Context(), GetFileSystemOperationOptions{
		Path:                               "v-Path",
		IncludeFiles:                       new(true),
		AllowFoldersWithoutTrailingSlashes: new(true),
	})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v1/filesystem")
	expectQuery(t, r, "path", "v-Path")
	expectQuery(t, r, "includeFiles", "true")
	expectQuery(t, r, "allowFoldersWithoutTrailingSlashes", "true")
