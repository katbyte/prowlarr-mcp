...
func TestOperationPostDownloadClient(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 201, "application/json", "{}")
	result, err := c.PostDownloadClient(t.Context(), DownloadClientResource{}, PostDownloadClientOperationOptions{
		ForceSave: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/downloadclient")
	expectQuery(t, r, "forceSave", "true")
	...
	if result.HttpResponse == nil || result.HttpResponse.StatusCode != 201 {
	...
	c, _ = newOperationServer(t, 201, "application/json", "<html>")
	result, err = c.PostDownloadClient(t.Context(), DownloadClientResource{}, PostDownloadClientOperationOptions{
		ForceSave: new(true),
	})
