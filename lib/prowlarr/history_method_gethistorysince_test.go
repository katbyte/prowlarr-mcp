...
func TestOperationGetHistorySince(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetHistorySince(t.Context(), GetHistorySinceOperationOptions{
		Date:      "v-Date",
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v1/history/since")
	expectQuery(t, r, "date", "v-Date")
