...
func TestOperationGetByIdDownload(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/octet-stream", "file bytes")
	result, err := c.GetByIdDownload(t.Context(), 7, GetByIdDownloadOperationOptions{
		Link: "v-Link",
		File: "v-File",
	})
	if err != nil {
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/7/download")
