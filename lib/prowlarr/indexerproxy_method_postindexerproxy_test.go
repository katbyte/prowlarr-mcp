...
func TestOperationPostIndexerProxy(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 201, "application/json", "{}")
	result, err := c.PostIndexerProxy(t.Context(), IndexerProxyResource{}, PostIndexerProxyOperationOptions{
		ForceSave: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/indexerproxy")
