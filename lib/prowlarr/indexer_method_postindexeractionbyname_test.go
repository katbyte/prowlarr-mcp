...
func TestOperationPostIndexerActionByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostIndexerActionByName(t.Context(), "p/name", IndexerResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/indexer/action/p%2Fname")
