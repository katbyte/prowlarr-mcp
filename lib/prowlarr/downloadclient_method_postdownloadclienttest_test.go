...
func TestOperationPostDownloadClientTest(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.PostDownloadClientTest(t.Context(), DownloadClientResource{}, PostDownloadClientTestOperationOptions{
		ForceTest: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/downloadclient/test")
