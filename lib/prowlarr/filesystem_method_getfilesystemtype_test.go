...
func TestOperationGetFileSystemType(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.GetFileSystemType(t.Context(), GetFileSystemTypeOperationOptions{
		Path: "v-Path",
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v1/filesystem/type")
