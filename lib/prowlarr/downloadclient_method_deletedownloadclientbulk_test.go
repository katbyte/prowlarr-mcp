...
func TestOperationDeleteDownloadClientBulk(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteDownloadClientBulk(t.Context(), DownloadClientBulkResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/api/v1/downloadclient/bulk")
