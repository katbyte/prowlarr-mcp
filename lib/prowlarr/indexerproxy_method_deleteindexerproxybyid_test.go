...
func TestOperationDeleteIndexerProxyById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "", "")
	result, err := c.DeleteIndexerProxyById(t.Context(), 7)
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodDelete, "/api/v1/indexerproxy/7")
