...
func TestOperationPutDownloadClientById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutDownloadClientById(t.Context(), 7, DownloadClientResource{}, PutDownloadClientByIdOperationOptions{
		ForceSave: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v1/downloadclient/7")
