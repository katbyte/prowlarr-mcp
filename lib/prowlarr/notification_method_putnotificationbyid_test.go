...
func TestOperationPutNotificationById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutNotificationById(t.Context(), 7, NotificationResource{}, PutNotificationByIdOperationOptions{
		ForceSave: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v1/notification/7")
