...
func TestOperationPutIndexerProxyById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutIndexerProxyById(t.Context(), 7, IndexerProxyResource{}, PutIndexerProxyByIdOperationOptions{
		ForceSave: new(true),
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v1/indexerproxy/7")
