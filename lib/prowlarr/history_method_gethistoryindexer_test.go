...
func TestOperationGetHistoryIndexer(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "[{}]")
	result, err := c.GetHistoryIndexer(t.Context(), GetHistoryIndexerOperationOptions{
		IndexerId: 7,
		EventType: HistoryEventTypeUnknown,
		Limit:     7,
	})
	if err != nil {
		t.Fatal(err)
	...
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v1/history/indexer")
	expectQuery(t, r, "indexerId", "7")
	expectQuery(t, r, "eventType", "unknown")
