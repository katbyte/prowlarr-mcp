...
func TestOperationPutConfigDownloadClientById(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 202, "application/json", "{}")
	result, err := c.PutConfigDownloadClientById(t.Context(), 7, DownloadClientConfigResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPut, "/api/v1/config/downloadclient/7")
