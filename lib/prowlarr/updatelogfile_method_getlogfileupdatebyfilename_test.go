...
func TestOperationGetLogFileUpdateByFilename(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "text/plain", "file bytes")
	result, err := c.GetLogFileUpdateByFilename(t.Context(), "p/filename")
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodGet, "/api/v1/log/file/update/p%2Ffilename")
