...
func TestOperationPostIndexerProxyActionByName(t *testing.T) {
	t.Parallel()

	c, s := newOperationServer(t, 200, "application/json", "{}")
	result, err := c.PostIndexerProxyActionByName(t.Context(), "p/name", IndexerProxyResource{})
	if err != nil {
		t.Fatal(err)
	}
	r, body := s.only(t)
	_ = body
	expectRequest(t, r, http.MethodPost, "/api/v1/indexerproxy/action/p%2Fname")
